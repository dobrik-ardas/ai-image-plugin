...
export const FromAiForm = ({ onClose, onAddAsset, trackedLocation }) => {
  ...
  const updateBalance = () => {
    request(`/ai/image/balance`, { method: 'GET' })
      ...
      .catch(() => {
        setError(<>Can't get balance, please check your API key in <Link href="/admin/settings/ai/image-generation">config section</Link></>)
      })
  }
  ...
  const handleSaveImages = () => {
    onAddAsset(blobsToAssets(checkedImages))
  }
...
FromAiForm.defaultProps = {
  trackedLocation: undefined,
};

FromAiForm.propTypes = {
